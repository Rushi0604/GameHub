...
class Admin:
    ...
    def add_product(self):
        ...
        image = st.file_uploader("Upload Product Image", type=["jpg"])
        if st.button("Add Product"):
            if name and price and description and image and rate and gtype and size is not None and discount is not None:
                conn = mysql.connector.connect(host="localhost",user="root",password="",database="epic")
                cursor = conn.cursor()
                cursor.execute("INSERT INTO games (g_type,g_name,g_price,g_size,g_rate,g_date,g_description,g_image,g_discount) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)",(gtype, name, price, size, rate,datetime.datetime.now().date(), description, image.read(), discount))
                conn.commit()
        ...
        if st.session_state.edit==True:
            ...
            if data=="g_rate" or data=="_type" or data=="g_name" or data=="g_price" or data=="g_size" or data=="description" or data=="g_discount" :
                ...
                if st.button("Update Product"):
                    cursor.execute(f"UPDATE games SET {data}=%s WHERE g_id=%s", (new_value, game_id))
                    conn.commit()
                ...
                if st.button("Update Product Image"):
                    cursor.execute("UPDATE games SET g_image=%s WHERE g_id=%s", (new_image.read(), game_id))
                    conn.commit()
    ...
    def delete_product(self,games_list):
        ...
        if st.button("Fetch Product Details"):
            ...
            self.view_products(game)
        if st.button("Delete Product"):
            cursor.execute("DELETE FROM games WHERE g_id=%s", (product_id,))
            conn.commit()
