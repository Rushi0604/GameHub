...
class Payment_methods:
    ...
    def call_type(self):
        if "email" in st.session_state or "phone" in st.session_state:    
            ...
            st.error("Please Login To Proceed With Payment")
        ...
        if cnum:
            if len(cnum) != 16 or not cnum.isdigit():
                ...
                if Payment_methods.check_num(cnum):
                    cursor = conn.cursor()
                    cursor.execute("SELECT * FROM details WHERE c_num = %s", (int(cnum),))
        ...
        if dnum:
            if len(dnum) != 16 or not dnum.isdigit():
                ...
                if Payment_methods.check_num(dnum):
                    cursor = conn.cursor()
                    cursor.execute("SELECT * FROM details WHERE d_num = %s", (int(dnum),))
                    data = cursor.fetchone()
                    cursor.close()
                    if data:
                        exp_month = data[3]
    ...
    def upi(total):
        ...
        conn=mysql.connector.connect(host="localhost", user="root", password="", database="bank")
        cursor=conn.cursor()
        cursor.execute("SELECT * FROM details WHERE upi_id=%s", (eupi,))
        reuslt=cursor.fetchone()
        st.session_state.payment=False
        if eupi and len(eupi)>0:
            if reuslt:
                if int(reuslt[7])>total:
                    if st.button("Confirm Payment"):
                        msg=st.empty()
                        msg.success("Payment Successfull")
                        time.sleep(2)
                        msg.empty()
                        Payment_methods.update_balance(total,reuslt[7],eupi,"upi_id")
                        st.session_state.payment=True
            ...
            if st.session_state.payment==True:
                Payment_methods.update_purchase_list(st.session_state.cart_list)
                pdf_file = Payment_methods.generate_invoice(reuslt[0], [(game.gname,game.gprice) for game in st.session_state.cart_list])
    ...
    def clear_cart():
        ...
        if "email" in st.session_state:
            cursor.execute("UPDATE user SET cart_list=%s WHERE user_email=%s", (" ",st.session_state.email,))
        else:
            cursor.execute("UPDATE user SET cart_list=%s WHERE user_phone=%s", (" ",st.session_state.phone,))
        conn.commit()
        cursor.close()
        conn.close()
    ...
    def update_purchase_list(new_games):
        ...
        if "email" in st.session_state:
            cursor.execute("UPDATE user SET purchase_list=%s WHERE user_email=%s",(updated_string, st.session_state.email),)
        else:
            cursor.execute("UPDATE user SET purchase_list=%s WHERE user_phone=%s",(updated_string, st.session_state.phone),)
        conn.commit()
        cursor.close()
        conn.close()
    ...
    def update_balance(total,balance,id,payment_type):
        ...
        if payment_type == "credit":
            cursor.execute("update details set balance=%s where c_num=%s",(balance-total,id))
        elif payment_type == "debit":
            cursor.execute("update details set balance=%s where d_num=%s",(balance-total,id))
        cursor.close()
        conn.commit()
        msg=st.empty()
        msg.success("Payment Successfull")
